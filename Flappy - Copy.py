...
def test_screen():
    running = True
    # Define the position of the anchor point (purple dot)
    anchor_x = screen_width // 2
    anchor_y = 100  # Position the dot near the top of the screen

    # Pendulum properties
    swing_angle = 0  # Initial angle
    swing_speed = 0.005  # Slower speed of swinging
    ...
    # Chain offset from the anchor point
    chain_offset_x = 11
    chain_offset_y = -30

    # Ensure the chain is scaled to its original size
    chain_img = pygame.image.load("chain.png").convert_alpha()
    chain_img = pygame.transform.scale(chain_img, (1000, chain_length))  # Adjust size as needed
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:  # Press ESC to return
                    return  # Exit the test screen
                if event.key == pygame.K_s:  # Press 'S' to toggle swinging
                    is_swinging = not is_swinging
                if event.key == pygame.K_UP:  # Move chain up
                    chain_offset_y -= 10
                if event.key == pygame.K_DOWN:  # Move chain down
                    chain_offset_y += 10
                if event.key == pygame.K_LEFT:  # Move chain left
                    chain_offset_x -= 10
                if event.key == pygame.K_RIGHT:  # Move chain right
                    chain_offset_x += 10
        ...
        # Calculate positions for the ball based on the swing angle
        ball_x = anchor_x + chain_offset_x + math.sin(swing_angle) * chain_length
        ball_y = anchor_y + chain_offset_y + math.cos(swing_angle) * chain_length

        # Draw the purple dot (anchor point)
        pygame.draw.circle(screen, (128, 0, 128), (anchor_x, anchor_y), 5)  # Purple dot

        # Rotate the chain image to match the swing angle
        rotated_chain_img = pygame.transform.rotate(chain_img, -math.degrees(swing_angle))
        chain_rect = rotated_chain_img.get_rect(midtop=(anchor_x + chain_offset_x, anchor_y + chain_offset_y))
        screen.blit(rotated_chain_img, chain_rect.topleft)

        # Rotate the ball image to simulate attachment
